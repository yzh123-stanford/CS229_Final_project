...
# Saving Results
metadata_path = os.path.join(save_dir, "results_metadata_custom_loss_simple_LSTM.json")
with open(metadata_path, "w") as f:
    json.dump(results_metadata, f, indent=4)
    
# Saving Models
for i, result in enumerate(hyperparameter_results):
    model_path = os.path.join(save_dir, f"best_model_state_BTCUSDT_custom_loss_simple_LSTM_{i}.pth")
...
# Saving Results
metadata_path = os.path.join(save_dir, "results_metadata_smoothL1_loss_simple_LSTM.json")
with open(metadata_path, "w") as f:
    json.dump(results_metadata, f, indent=4)
    
# Saving Models
for i, result in enumerate(hyperparameter_results):
    model_path = os.path.join(save_dir, f"best_model_state_BTCUSDT_smoothL1_loss_simple_LSTM_{i}.pth")
    torch.save(result["best_model_state"], model_path)
